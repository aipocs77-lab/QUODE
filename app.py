...
from rag_pipeline.ingest import ingest_all, is_chroma_corruption_error, reset_chroma_store
...
from rag_pipeline.token_cache import get_metrics as get_cache_metrics, reset as reset_cache, get_cache_size
...
_SUPPORTED   = ["xlsm", "xlsx", "txt", "png", "jpg", "jpeg", "svg"]
...
_DEFAULTS: dict[str, Any] = {
    ...
    # Diagram data persisted across turns for multi-turn analysis
    "asis_diagram_data": None,  # raw DOT text from As-Is questionnaire
    "tobe_diagram_data": None,  # raw DOT text from LLM To-Be generation
...
with st.sidebar:
    ...
    # ── Token Count ───────────────────────────────────────────────────────
    with st.expander("🔢 Token Counts", expanded=False):
        render_token_counter(
    ...
    with st.expander("⚡ Cache Monitor", expanded=False):
        _cm = get_cache_metrics()
        _cs = get_cache_size()
        _hit_color = "#10b981" if _cm.hit_rate >= 50 else ("#f59e0b" if _cm.hit_rate >= 20 else "#ef4444")
        st.markdown(
            f"""
            <div style="display:grid;grid-template-columns:1fr 1fr;gap:6px;padding:4px 0 8px;">
              <div style="background:#161b22;border-radius:6px;padding:8px;text-align:center;">
                <div style="font-size:1.1rem;font-weight:700;color:{_hit_color};">{_cm.hit_rate:.0f}%</div>
                <div style="font-size:0.65rem;color:#8b949e;">Hit Rate</div>
              </div>
              <div style="background:#161b22;border-radius:6px;padding:8px;text-align:center;">
                <div style="font-size:1.1rem;font-weight:700;color:#e6edf3;">{_cs}</div>
                <div style="font-size:0.65rem;color:#8b949e;">Cached</div>
              </div>
              <div style="background:#161b22;border-radius:6px;padding:8px;text-align:center;">
                <div style="font-size:1.1rem;font-weight:700;color:#58a6ff;">{_cm.hits}</div>
                <div style="font-size:0.65rem;color:#8b949e;">Hits</div>
              </div>
              <div style="background:#161b22;border-radius:6px;padding:8px;text-align:center;">
                <div style="font-size:1.1rem;font-weight:700;color:#484f58;">{_cm.misses}</div>
                <div style="font-size:0.65rem;color:#8b949e;">Misses</div>
              </div>
            </div>
            <div style="background:#161b22;border-radius:6px;padding:8px;text-align:center;margin-bottom:6px;">
              <div style="font-size:1.0rem;font-weight:700;color:#10b981;">
                {_cm.tokens_saved:,}
              </div>
              <div style="font-size:0.65rem;color:#8b949e;">Tokens Saved by Cache</div>
            </div>
            """,
            unsafe_allow_html=True,
        )
        if st.button("↺ Reset Cache", use_container_width=True, key="reset_cache"):
            reset_cache()
            st.rerun()
    ...
    with st.expander("📂 Upload Documents", expanded=True):
        uploaded_files = st.file_uploader(
            "xlsm · xlsx · txt · png · jpg · svg",
            type=_SUPPORTED,
            accept_multiple_files=True,
            help="Upload your QODE questionnaire (.xlsm/.xlsx), plain text, "
            "or images (.png/.jpg/.svg) — image text is extracted via OCR.",
    ...
    # ── Resolve paths (outside expander — needed by ingest + chain) ────────
    ...
    if not st.session_state.ingested:
        with st.spinner("🔄 Re-concilling Knowledge Base"):
            try:
                ...
                if is_chroma_corruption_error(exc):
                    st.warning(
                        "Detected a corrupted local vector index. Rebuilding ChromaDB and retrying ingestion."
                    )
                    try:
                        reset_chroma_store(_CHROMA_PATH)
                        n_docs = ingest_all(
                            excel_path=excel_path,
                            chroma_path=_CHROMA_PATH,
                            extra_file_paths=extra_docs,
                        ...
                        st.error(f"Ingestion error after rebuild: {retry_exc}")
                else:
    ...
    with st.expander("⚡ One Touch As-Is Diagram", expanded=True):
    ...
    with st.expander("🔮 One Touch To-Be Diagram", expanded=True):
    ...
    with st.expander("📜 Chat History (96 h)", expanded=False):
        try:
        ...
        if not _sessions:
            ...
            for _si, _sess in enumerate(_sessions):
                ...
                _raw_title  = _sess["first_message"] or "(empty session)"

                # Truncate at word boundary ≤ 60 chars
                if len(_raw_title) > 60:
                    _title = _raw_title[:60].rsplit(" ", 1)[0] + " …"
                else:
                    _title = _raw_title
                ...
                st.markdown(
                    ...
                    f'title="{html.escape(_raw_title)}">{html.escape(_title)}</div>'
                    f'</div>',
                    unsafe_allow_html=True,
                )
                if not _is_current:
...
_token_pct = (
    st.session_state.token_total / st.session_state.token_budget * 100
    if st.session_state.token_budget > 0 else 0
)
if _token_pct >= 95:
    st.error(
        f"🔴 **Token budget critical** — {_token_pct:.0f}% used "
        f"({st.session_state.token_total:,} / {st.session_state.token_budget:,}). "
        "Reset the counter in the sidebar or responses may be truncated.",
        icon="⛔",
    )
elif _token_pct >= 80:
    st.warning(
        f"⚠️ **Token budget at {_token_pct:.0f}%** — "
        f"{st.session_state.token_total:,} of {st.session_state.token_budget:,} tokens used. "
        "Consider resetting the counter in the sidebar.",
        icon="⚠️",
    )
...
if prompt:
    ...
    # Narrative check first — planning/analysis queries must never show diagram status
    status_msg = (
        "📝 Preparing analysis …"
        if is_narrative
        else f"📊 Creating As-Is {intent.title()} Architecture codebase …"
    ...
    with st.chat_message("assistant"):
        with st.status(status_msg, expanded=False):
            try:
                ...
                result = run_chain(
                    ...
                    stream=use_stream,
                    asis_diagram_data=st.session_state.asis_diagram_data,
                    tobe_diagram_data=st.session_state.tobe_diagram_data,
                )
            except Exception as exc:
                result = {
        ...
        # Persist diagram data for next turn (multi-turn analysis)
        if result.get("asis_diagram_data"):
            st.session_state.asis_diagram_data = result["asis_diagram_data"]
        if result.get("tobe_diagram_data"):
            st.session_state.tobe_diagram_data = result["tobe_diagram_data"]
        ...
        if result.get("stream"):
            ...
            if is_narrative and full_reply and result.get("mode") != "error":
                # Render full response as formatted markdown (preserves tables, bold, lists)
                st.markdown(display_text)

                st.markdown(
                    "<div style='font-size:0.82rem;color:#8b949e;margin:8px 0 4px;'>"
                    "📄 <strong>Export full analysis:</strong></div>",
                    unsafe_allow_html=True,
                )
                _doc_title = prompt[:80]
                _report_images = result.get("report_images")
                _dcols = st.columns(4)
                _doc_fmts = [
                    ("word",  "📝 Word (.docx)",   ".docx"),
                ...
                for _col, (_fmt, _label, _ext) in zip(_dcols, _doc_fmts):
                    with _col:
                        try:
                            _data, _fname, _mime = generate_document(_fmt, _doc_title, full_reply, images=_report_images)
                            st.download_button(
                                label=_label,
                                data=_data,
                                file_name=_fname,
