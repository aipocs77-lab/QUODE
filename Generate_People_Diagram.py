    ...
    def create_edge(self, final_df):
        for i in range(0, final_df.shape[0]):
            ...
            if self.isnan(pred_1) == False and pred_1 != "INIT":
                matches = final_df.loc[final_df['S#'] == pred_1, 'Team / owner role']
                if len(matches) > 0:
                    pred_role = matches.iloc[0]
                    if pred_role in self.node_list:
            ...
            if self.isnan(pred_2) == False and pred_2 != "INIT":
                matches = final_df.loc[final_df['S#'] == pred_2, 'Team / owner role']
                if len(matches) > 0:
                    pred_role = matches.iloc[0]
                    if pred_role in self.node_list:
                        self.edge_creation(self.node_list[pred_role]["value"],self.node_list[current_role]["value"], color)
            if self.isnan(pred_3) == False and pred_3 != "INIT":
                matches = final_df.loc[final_df['S#'] == pred_3, 'Team / owner role']
                if len(matches) > 0:
                    pred_role = matches.iloc[0]
                    if pred_role in self.node_list:
                        self.edge_creation(self.node_list[pred_role]["value"],self.node_list[current_role]["value"], color)
            if self.isnan(pred_4) == False and pred_4 != "INIT":
                matches = final_df.loc[final_df['S#'] == pred_4, 'Team / owner role']
                if len(matches) > 0:
                    pred_role = matches.iloc[0]
                    if pred_role in self.node_list:
                        self.edge_creation(self.node_list[pred_role]["value"],self.node_list[current_role]["value"], color)
            if self.isnan(pred_5) == False and pred_5 != "INIT":
                matches = final_df.loc[final_df['S#'] == pred_5, 'Team / owner role']
                if len(matches) > 0:
                    pred_role = matches.iloc[0]
                    if pred_role in self.node_list:
                        self.edge_creation(self.node_list[pred_role]["value"],self.node_list[current_role]["value"], color)
