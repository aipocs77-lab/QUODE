...
def _validate_grounding(chunks: list[dict], asis_data: str | None, is_asis: bool, is_gap: bool) -> tuple[bool, str]:
    """Validate whether questionnaire/graph retrieval can ground the response."""
    ...
    valid_chunks = [
        ...
        and c.get("text")
        and len(str(c.get("text", "")).strip()) > 10
    ]
    ...
    if is_asis or is_gap:
        ...
        if has_quality_chunks:
            logger.info(
                "As-Is grounded in %d retrieved questionnaire/graph chunks",
                len(valid_chunks),
            )
            return True, (
                f"As-Is grounded in {len(valid_chunks)} "
                "retrieved questionnaire/graph chunks"
            )
...
def _node_principles(state: AgentState) -> AgentState:
    ...
    with tracer.trace("context_quality_check", input={"intent": intent, "chunk_count": len(chunks)}) as span:
        try:
            ...
            _evaluation_graph = load_graph(state["graph_path"])
            if _evaluation_graph is not None:
                graph_data = _evaluation_graph.snapshot_data()
        except Exception as e:
