...
class _DirectedGraph:
    """Minimal directed-graph compatibility layer without NetworkX."""
    ...
    def add_edge(self, source, target, **attrs):
        ...
        self._edge_data[(source, target)] = dict(attrs)
    ...
    def get_edge_data(self, source, target, default=None):
        return self._edge_data.get((source, target), default)


def _descendants(graph, node):
    seen, frontier = set(), list(graph._succ.get(node, ()))
    while frontier:
        current = frontier.pop()
        if current in seen:
            continue
        seen.add(current)
        frontier.extend(graph._succ.get(current, ()))
    return seen


def _ancestors(graph, node):
    seen, frontier = set(), list(graph._pred.get(node, ()))
    while frontier:
        current = frontier.pop()
        if current in seen:
            continue
        seen.add(current)
        frontier.extend(graph._pred.get(current, ()))
    return seen
...
def _all_simple_paths(graph, source, target):
    ...
    def visit(node, path):
        if node == target:
            paths.append(path[:])
            return
        for nxt in graph._succ.get(node, ()):
            if nxt not in path:
                visit(nxt, path + [nxt])
    visit(source, [source])
    return paths
...
class Process_Diagram:
    ...
    def find_leafnodes(self, G):
        ...
        for i in G.nodes:
            head =  []
            if _descendants(G, i) == set(): 
                for a in _ancestors(G, i):  
                    if _ancestors(G, a) == set():  
                        head.append(a)
            if len(head) == 1: 
                leafnode.append(i)
        return leafnode
    ...
    def find_critical_paths(self, final_df, node_list):
        ...
        for node in final_leaf_node:
            paths = sorted(_all_simple_paths(self.Qode_graph, source= 'E0', target= node))
